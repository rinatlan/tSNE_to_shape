...
class Shape:
    ...
    def point_to_color(self, point_to_examine, shape, cmap, segments_arr):
        distance = self.min_dist_point_to_shape(point_to_examine, shape, segments_arr)
    ...
    def min_dist_point_to_shape(self, point_to_examine, shape, segments_arr):
        if shape.contains_point(point_to_examine):
            return 0.0
        else:
            # distance = np.empty(self.curve_pts[:, [0, 1]].shape, dtype=np.float64)
            min_dist = np.inf
            # for ((x, y), code) in shape.iter_segments(simplify=False, curves=True):
            for ((x, y), code) in segments_arr:
                # try to make a matrix of the (x,y) in all the iterations,
                # then after the for loop do the temp_dist calculation (matrix peulot)

                temp_dist = np.sqrt((y - point_to_examine[1]) ** 2 +
                                    (x - point_to_examine[0]) ** 2)
    ...
    def generate_image(self, *args, **kwargs):
        ...
        xmin = kwargs.get('xmin', -100.0)
        xmax = kwargs.get('xmax', 100.0)
        ymin = kwargs.get('ymin', -100.0)
        ymax = kwargs.get('ymax', 100.0)
        ...
        # Rinat's changes
        shape = matplotlib.path.Path(self.curve_pts[:, [0, 1]])

        num_points_to_base_on = 100
        segments_arr = shape.iter_segments(simplify=False, curves=True)

        point_to_examine = np.array([0, 0.5])
        new_color = self.point_to_color(point_to_examine, shape, self.cmap, segments_arr)
        plt.scatter(point_to_examine[0], point_to_examine[1], color=new_color,
                    s=50, zorder=2, alpha=1.0)

        point_to_examine = np.array([0, 1])
        # shape = matplotlib.path.Path(self.curve_pts[:, [0, 1]])
        new_color = self.point_to_color(point_to_examine, shape, self.cmap, segments_arr)
        plt.scatter(point_to_examine[0], point_to_examine[1], color=new_color,
                    s=50, zorder=2, alpha=1.0)

        point_to_examine = np.array([1.5, 0])
        # shape = matplotlib.path.Path(self.curve_pts[:, [0, 1]])
        new_color = self.point_to_color(point_to_examine, shape, self.cmap, segments_arr)
        plt.scatter(point_to_examine[0], point_to_examine[1], color=new_color,
                    s=50, zorder=2, alpha=1.0)

        point_to_examine1 = np.array([0, 2])
        # shape = matplotlib.path.Path(self.curve_pts[:, [0, 1]])
        new_color1 = self.point_to_color(point_to_examine1, shape, self.cmap, segments_arr)
        plt.scatter(point_to_examine1[0], point_to_examine1[1], color=new_color1,
                    s=50, zorder=2, alpha=1.0)
